...
def login():
    ...
    return render_template("auth/login.html", api_base=_api_base(), title="Đăng nhập")

@bp.route("/register")
def register():
    return render_template("auth/register.html", api_base=_api_base(), title="Đăng ký")

@bp.route("/dashboard")
def dashboard():
    # Không kiểm tra session ở server. JS sẽ guard bằng token trong localStorage.
    return render_template("dashboard.html", api_base=_api_base(), title="Dashboard")
